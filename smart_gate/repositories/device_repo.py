...
class DeviceRepository:
    ...
    def upsert_device(self, device: DeviceConfig) -> None:
        ...
        self.conn.execute(
            ...
            INSERT INTO local_device_config (
                device_id, device_name, gate_id, lane_id, mac_address, access_token, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def save_user_profile(self, profile: UserProfile) -> None:
        ...
        self.conn.execute(
            """
            INSERT INTO local_user_profile (id, email, full_name, role, updated_at)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(id) DO UPDATE SET
                ...
                updated_at=excluded.updated_at
            """,
            (profile.id, profile.email, profile.full_name, profile.role, now),
        )
        self.conn.commit()
    ...
    def clear_session(self) -> None:
        self.conn.execute("UPDATE local_device_config SET access_token=NULL")
